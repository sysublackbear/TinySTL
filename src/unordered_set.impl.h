...
// 关键结构：TinySTL::vector<TinySTL::list<key_type>> buckets_;
// 关键通过load_factor是否大于max_load_factor，决定哈希表是否需要扩容
namespace TinySTL{
	namespace Detail{
	...
	typename Unordered_set<Key, Hash, KeyEqual, Allocator>::size_type
		...
		return bucket_index(key);  // 求出某个key对应的bucket下标
	}
	// load_factor：装载因子
	template<class Key, class Hash, class KeyEqual, class Allocator>
	float Unordered_set<Key, Hash, KeyEqual, Allocator>::load_factor()const{
		return (float)size() / (float)bucket_count();  // bucket_count()——buckets_.size(); 桶的数量
		// 占用的桶数量除以总的桶数量，如果超过了装载因子(load_factor)，则自动扩容
	}
	template<class Key, class Hash, class KeyEqual, class Allocator>
	...
	Unordered_set<Key, Hash, KeyEqual, Allocator>::Unordered_set(size_type bucket_count){
		// 注意分配容量的时候，需要比既定的容量要大，但这里不是简单的翻倍操作，哈希表的key值最好为质数（减少冲突率）
		// 所以这里用数组记了一个质数表，在里面找下一个更大的质数。
	...
	TinySTL::pair<typename Unordered_set<Key, Hash, KeyEqual, Allocator>::iterator, bool> 
		...
		if (!has_key(val)){
			if (load_factor() > max_load_factor())  // 目前装载因子>最大装载因子
				rehash(next_prime(size()));  // 对哈希表进行扩容
