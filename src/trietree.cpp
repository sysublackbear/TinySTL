...
namespace TinySTL{
	...
	bool trie_tree::insert(const string& word){
		...
		if (res != root->map_childs.end()){
			...
			// unique_ptr虽然没有支持普通的拷贝和赋值操作，
			// 但却提供了一种移动机制来将指针的所有权从一个unique_ptr转移给另一个unique_ptr。如果需要转移所有权，可以使用std::move()函数。
			root->map_childs[ch] = std::move(node);  // 转移构造,左值转成一个右值
			// 转移构造的底层原理？
	...
	bool trie_tree::_insert(const string& word, const node_ptr& up){
		if (word.empty()){
			++size_;
			up->is_a_word = true;  // 到word的末尾，它是一个word,is_a_word打上true
			return true;
		}
		char ch = word[0];
		auto res = up->map_childs.find(ch);
