...
namespace TinySTL{
	...
	void vector<T, Alloc>::reallocateAndCopy(iterator position, InputIterator first, InputIterator last){
		difference_type newCapacity = getNewCapacity(last - first);  // 求出n和当前容量的较大值
	...
	void vector<T, Alloc>::reallocateAndFillN(iterator position, const size_type& n, const value_type& val){
		difference_type newCapacity = getNewCapacity(n);  // 求出n和当前容量的较大值
	...
	void vector<T, Alloc>::destroyAndDeallocateAll(){
		if (capacity() != 0){
			dataAllocator::destroy(start_, finish_);  // 先析构对象，调用Allocate::destroy
			dataAllocator::deallocate(start_, capacity());  // 再将空间回收到内存池中,调用Allocate::deallocate
		}
	}
	template<class T, class Alloc>
